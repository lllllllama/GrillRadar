# GrillRadar 外部信息源集成

> **Milestone 4**: 外部信息源集成（MVP版本）

本文档说明 GrillRadar 的外部信息源集成功能，包括 JD（职位描述）和面经（面试经历分享）的检索和使用。

---

## 概述

Milestone 4 引入了外部信息源集成，使报告生成能够参考真实的 JD 和面经数据，生成更贴近实际面试场景的问题。

### 核心功能

- ✅ **JD 检索** - 检索目标公司和岗位的职位描述
- ✅ **面经检索** - 检索相关的面试经历分享
- ✅ **信息聚合** - 自动提取和聚合关键信息（技能要求、高频问题等）
- ✅ **趋势洞察** - 通过 LocalDatasetProvider + JSON 数据库返回结构化高频技能/主题
- ✅ **Prompt 注入** - 将外部信息自动注入到报告生成 Prompt 中
- ✅ **API 支持** - 提供 REST API 查询外部信息与趋势

---

## 架构设计

```
┌─────────────────────┐
│  UserConfig         │
│  (enable_external   │
│   _info: true)      │
└──────────┬──────────┘
           │
           v
┌─────────────────────┐
│ ExternalInfoService │ ← uses → MockDataProvider / LocalDatasetProvider / MultiSourceCrawler
└──────────┬──────────┘
           │
           v
┌─────────────────────┐
│  InfoAggregator     │
│  (提取+聚合)         │
└──────────┬──────────┘
           │
           v
┌─────────────────────┐
│ ExternalInfoSummary │
└──────────┬──────────┘
           │
           v
┌─────────────────────┐
│  PromptBuilder      │
│  (注入到Prompt)     │
└─────────────────────┘
```

---

## 数据模型

### 1. JobDescription（职位描述）

```python
class JobDescription(BaseModel):
    company: str              # 公司名称
    position: str             # 职位名称
    location: Optional[str]   # 工作地点
    salary_range: Optional[str]  # 薪资范围
    requirements: List[str]   # 岗位要求列表
    responsibilities: List[str]  # 工作职责列表
    keywords: List[str]       # 提取的关键技能
    source_url: Optional[str] # 来源URL
    crawled_at: Optional[datetime]  # 爬取时间
```

### 2. InterviewExperience（面经）

```python
class InterviewExperience(BaseModel):
    company: str              # 公司名称
    position: str             # 职位名称
    interview_type: str       # 面试类型（一面/二面/HR面等）
    questions: List[str]      # 面试问题列表
    difficulty: Optional[str] # 难度（简单/中等/困难）
    topics: List[str]         # 涉及的技术主题
    tips: Optional[str]       # 面试建议
    source_url: Optional[str] # 来源URL
    shared_at: Optional[datetime]  # 分享时间
```

### 3. ExternalInfoSummary（信息摘要）

```python
class ExternalInfoSummary(BaseModel):
    job_descriptions: List[JobDescription]
    interview_experiences: List[InterviewExperience]
    aggregated_keywords: List[str]        # 聚合的关键词
    aggregated_topics: List[str]          # 聚合的主题
    high_frequency_questions: List[str]   # 高频问题
    keyword_trends: List[KeywordTrend]    # 高频技能趋势（来源+频次）
    topic_trends: List[TopicTrend]        # 热点面试主题
    retrieved_at: datetime                # 检索时间
```

---

## 使用方式

### CLI 方式

在配置文件中启用外部信息源：

```json
{
  "mode": "job",
  "target_desc": "字节跳动后端开发工程师",
  "domain": "backend",
  "enable_external_info": true,
  "target_company": "字节跳动"
}
```

### Web API 方式

```bash
curl \
  "http://localhost:8000/api/external-info/search?company=字节跳动&position=后端&domain=backend"

# 获取结构化趋势，可选触发新一轮检索
curl "http://localhost:8000/api/external-info/trends?domain=backend"
```

在请求中包含外部信息字段：

```bash
curl -X POST http://localhost:8000/api/generate-report \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "job",
    "target_desc": "字节跳动后端开发工程师",
    "domain": "backend",
    "resume_text": "...",
    "enable_external_info": true,
    "target_company": "字节跳动"
  }'
```

### 查询外部信息

```bash
# 搜索外部信息
curl "http://localhost:8000/api/external-info/search?company=字节跳动&position=后端"

# 预览外部信息摘要
curl "http://localhost:8000/api/external-info/preview?company=字节跳动"
```

---

## Mock 数据提供者

**当前版本使用 Mock 数据（演示模式）**

为了演示功能而不依赖真实的网络爬虫，Milestone 4 MVP 版本使用模拟数据：

- **MockDataProvider** - 提供预定义的 JD 和面经数据
- 涵盖字节跳动、阿里巴巴、腾讯等公司
- 包含后端、Java、C++ 等常见岗位

### Mock 数据示例

**字节跳动 - 后端开发工程师 JD:**
- 要求：3年以上经验、精通Java/Go、熟悉分布式系统
- 技术栈：Java, Go, MySQL, Redis, Kafka, Docker, Kubernetes

**字节跳动 - 后端面经（一面）:**
- 详细讲一下你的分布式爬虫系统是如何设计的？
- 如何保证分布式任务不重复、不丢失？
- Redis和MySQL的数据一致性如何保证？
- 如果让你设计一个秒杀系统，你会怎么做？

---

## 扩展为真实数据源

将 Mock 提供者替换为真实爬虫：

```python
# app/sources/real_crawler.py
class RealDataProvider:
    def get_jds(self, company, position):
        # 实现真实的JD爬虫
        # 可使用：
        # - 拉勾网 API
        # - Boss直聘 API
        # - WebSearch 工具
        pass

    def get_experiences(self, company, position):
        # 实现真实的面经爬虫
        # 可使用：
        # - 牛客网
        # - LeetCode论坛
        # - 知乎
        pass
```

然后在 `ExternalInfoService` 中：

```python
self.real_provider = RealDataProvider()

if self.use_mock:
    jds = self.mock_provider.get_mock_jds(company, position)
else:
    jds = self.real_provider.get_jds(company, position)  # 真实爬虫
```

---

## API 端点

### 1. GET /api/external-info/search

搜索外部信息（返回完整的 JSON 对象）

**参数:**
- `company` (可选): 目标公司
- `position` (可选): 目标岗位
- `enable_jd` (默认true): 是否启用JD检索
- `enable_interview_exp` (默认true): 是否启用面经检索

**响应:** `ExternalInfoSummary` JSON

### 2. GET /api/external-info/preview

预览外部信息（返回文本摘要）

**参数:**
- `company` (可选): 目标公司
- `position` (可选): 目标岗位

**响应:**
```json
{
  "summary": "格式化的文本摘要",
  "jd_count": 3,
  "experience_count": 5,
  "keywords": ["Java", "Go", "分布式", ...]
}
```

---

## 对报告生成的影响

启用外部信息源后，报告生成 Prompt 会包含：

1. **核心技能要求** - 从多个JD中聚合的关键词
2. **典型JD示例** - 显示部分真实JD的要求
3. **高频考察主题** - 从面经中提取的常见主题
4. **高频面试题示例** - 真实面经中出现的问题

这使得生成的问题：
- ✅ 更贴近真实面试场景
- ✅ 包含该公司/岗位的特定要求
- ✅ 覆盖高频考察点
- ✅ 提供真实面试题作为参考

---

## 注意事项

### MVP 版本限制

- 🔸 **仅支持 Mock 数据** - 需要手动扩展为真实爬虫
- 🔸 **简单的关键词提取** - 未使用 NLP 技术
- 🔸 **有限的去重和聚类** - 高频问题提取较简单

### 扩展建议

1. **实现真实爬虫**
   - 使用合法的API（如智联招聘、拉勾网开放API）
   - 尊重 robots.txt
   - 添加请求限流

2. **增强 NLP 处理**
   - 使用 jieba 进行中文分词
   - TF-IDF 提取关键词
   - 聚类相似问题

3. **缓存机制**
   - Redis 缓存检索结果
   - 定期更新数据

4. **数据质量控制**
   - 过滤低质量数据
   - 验证数据完整性
   - 标注数据来源可信度

---

## 文件结构

```
app/
├── models/
│   └── external_info.py       # 数据模型
├── sources/
│   ├── __init__.py
│   ├── mock_provider.py       # Mock数据提供者
│   └── external_info_service.py  # 外部信息服务
├── retrieval/
│   ├── __init__.py
│   └── info_aggregator.py     # 信息聚合器
└── api/
    └── report.py              # API端点（已增强）
```

---

**Last Updated**: 2025-11-17  
**Version**: 1.0 (Milestone 4 MVP)  
**Status**: Demo/Mock 模式（可扩展）
