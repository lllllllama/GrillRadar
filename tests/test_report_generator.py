...
class TestReportGeneration:
    ...
    def test_generate_report_llm_error(self, mock_llm_client, sample_user_config):
        ...
        assert isinstance(report, Report)
        assert "报告生成失败" in report.questions[0].question  # Error info is in the question
        assert "LLM API Error" in report.questions[0].rationale
...
class TestReportValidation:
    ...
    def test_validate_too_many_questions(self, mock_llm_client, sample_user_config, create_report_data):
        """Test that excessive questions are smartly truncated instead of failing"""
        ...
        # No error message in summary for success case
        assert "简化报告" not in report.summary
